...
namespace Menus
{
	namespace detail
	{
		...
		std::uint32_t GetFilterFlag(const RE::BGSInventoryItem* a_inventoryItem, [[maybe_unused]] const RE::BGSInventoryItem::Stack* a_stack)
		{
			...
			case RE::ENUM_FORM_ID::kMISC:  // Misc, Junk, Mods
				{
					...
					if (misc)
					{
						...
						auto looseMod = Forms::LooseModKeyword_DO->GetForm<RE::BGSKeyword>();
						if (looseMod && misc->HasKeyword(looseMod, nullptr))
						{
